...
const SearchResults = ({ eventsData }) => {
  ...
  const default_image = 'https://eventhub12.blob.core.windows.net/images/default.jpg?sp=r&st=2024-03-18T06:52:24Z&se=2024-03-24T14:52:24Z&spr=https&sv=2022-11-02&sr=b&sig=nWb0Dzb9%2FWPfAZ6X5MRrwoi%2FxHU8OLe0I6nPtwpBkbQ%3D';
            ...
            <div className={styles.EventDescription}>
              <div className={styles.titleAndParticipants}>
                ...
                <div className={`${styles.details} ${styles.iconTextContainer}`}>
                  ...
                  <p>{event.participant_count}/{event.max_participants}</p>
                </div>
              </div>
              <div className={`${styles.details} ${styles.iconTextContainer}`}>
                <CalendarOutlined className={styles.icon} />
                <p>{new Date(event.start_at).toLocaleDateString()} - {new Date(event.expire_at).toLocaleDateString()}</p>
              </div>
              <div className={`${styles.details} ${styles.iconTextContainer}`}>
                <ClockCircleOutlined className={styles.icon} />
                <p>              {new Date(event.start_at).toLocaleTimeString().slice(0, 5)} - {new Date(event.expire_at).toLocaleTimeString().slice(0, 5)}
                </p>
              </div>
              <div className={`${styles.details} ${styles.iconTextContainer}`}>
